...
static void _spi_configure_pinmux(spi_target_t target, bool use_for_spi)
{
	switch (target) {
		...
		case SPI_FPGA_JTAG:
			if (use_for_spi) {
				gpio_set_pin_function(PIN_PA06, MUX_PA06C_SERCOM0_PAD0);
				gpio_set_pin_function(PIN_PA07, MUX_PA07C_SERCOM0_PAD1);
				gpio_set_pin_function(PIN_PA04, MUX_PA04C_SERCOM0_PAD2);
			} else {
				gpio_set_pin_function(PIN_PA06, GPIO_PIN_FUNCTION_OFF);
				gpio_set_pin_function(PIN_PA07, GPIO_PIN_FUNCTION_OFF);
